...
func readStudent() -> Student? {
    ...
    guard temp.count == 4,
        ...
        let math = Int(temp[3]) else { return nil }
    let score = Score(korean: korean, english: english, math: math)
    return Student(name: String(temp[0]), score: score)
}

func solution(studnets: [Student]) -> String {
    return studnets.sorted().map{$0.description}.joined(separator: "\n")
}
